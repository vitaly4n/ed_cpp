...
Stop
Stop::ParseFrom(const Json::Dict& attrs)
{
...
vector<string>
ParseStops(const vector<Json::Node>& stop_nodes, bool is_roundtrip)
{
  vector<string> stops;
...
int
ComputeStopsDistance(const Stop& lhs, const Stop& rhs)
{
...
Bus
Bus::ParseFrom(const Json::Dict& attrs)
{
  Bus bus{ .name = attrs.at("name").AsString(),
           .stops = ParseStops(attrs.at("stops").AsArray(), attrs.at("is_roundtrip").AsBool()),
           .routing_stops = ParseStops(attrs.at("stops").AsArray(), true) };

  bus.start_stop = *begin(bus.routing_stops);
  bus.end_stop = *rbegin(bus.routing_stops);
  return bus;
}

vector<InputQuery>
ReadDescriptions(const vector<Json::Node>& nodes)
{
