...
template<typename T>
void
PrintCoeff(std::ostream& out, int i, const T& coef, bool printed)
{
...
template<typename T>
std::ostream&
operator<<(std::ostream& out, const Polynomial<T>& p)
{
  bool printed = false;
...
template<typename T>
Polynomial<T>
operator+(Polynomial<T> lhs, const Polynomial<T>& rhs)
{
  lhs += rhs;
  return lhs;
}
...
TestCreation()
{
  ...
  }
  {
    ...
    ASSERT(
      std::equal(cbegin(from_iterators), cend(from_iterators), begin(coeffs)));
  }
}
