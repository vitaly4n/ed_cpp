...
std::ostream&
operator<<(std::ostream& output, Position pos)
{
...
std::ostream&
operator<<(std::ostream& output, Size size)
{
  return output << "(" << size.rows << ", " << size.cols << ")";
}

std::ostream&
operator<<(std::ostream& output, const ICell::Value& value)
{
...
std::string_view
ToString(IFormula::HandlingResult hr)
{
  switch (hr) {
...
std::ostream&
operator<<(std::ostream& output, IFormula::HandlingResult hr)
{
...
namespace {
std::string
ToString(FormulaError::Category category)
{
...
void
TestFormulaArithmetic()
{
  auto sheet = CreateSheet();
  auto evaluate = [&](std::string expr) { return std::get<double>(ParseFormula(std::move(expr))->Evaluate(*sheet)); };
...
void
TestFormulaExpressionFormatting()
{
...
TestCellErrorPropagation()
{
  ...
  ASSERT(value == ICell::Value(FormulaError::Category::Ref) || value == ICell::Value(FormulaError::Category::Div0));
}

void
TestCellsDeletionSimple()
{
  auto sheet = CreateSheet();
  sheet->SetCell("A1"_pos, "1");
  sheet->SetCell("A2"_pos, "2");
