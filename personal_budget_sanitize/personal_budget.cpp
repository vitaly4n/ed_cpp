...
void
ValidateBounds(Number number_to_check, Number min_value, Number max_value)
{
...
ostream&
operator<<(ostream& stream, const MoneyState& state)
{
...
IndexSegment
IntersectSegments(IndexSegment lhs, IndexSegment rhs)
{
...
bool
AreSegmentsIntersected(IndexSegment lhs, IndexSegment rhs)
{
...
template<typename Data, typename BulkOperation>
using SummingSegmentTreeHolder =
  std::shared_ptr<SummingSegmentTree<Data, BulkOperation>>;
...
public:
  ...
  BaseBulkOperation(const TreeHolder& tree)
    : tree_(tree)
  {}
...
{
public:
  BulkLinearUpdater(const TreeHolder& tree = nullptr)
...
public:
  ...
  {
    this->TraverseWithQuery(
      root_.get(), segment, AddBulkOperationVisitor{ operation });
  }
...
private:
  ...
  TraverseWithQuery(Node* node, IndexSegment query_segment, Visitor visitor)
  {
  ...
  public:
    ...
    Data ProcessEmpty(Node*) const { return {}; }

    Data ProcessFull(Node* node) const { return node->data; }

    Data ProcessPartial(Node*,
                        IndexSegment,
...
size_t
ComputeDaysDiff(const Date& date_to, const Date& date_from)
{
...
size_t
ComputeDayIndex(const Date& date)
{
  return ComputeDaysDiff(date, START_DATE);
}

IndexSegment
MakeDateSegment(const Date& date_from, const Date& date_to)
{
...
public:
  ...
  BudgetManager()
    : tree_(new SummingSegmentTree<MoneyState, BulkLinearUpdater>(DAY_COUNT))
  {}
  const TreeHolder& GetTree() const { return tree_; }
...
struct ComputeIncomeRequest : ReadRequest<double>
{
  ...
  double Process(const BudgetManager& manager) const override
  {
    return manager.GetTree()
      ->ComputeSum(MakeDateSegment(date_from, date_to))
      .ComputeIncome();
  }
...
struct AddMoneyRequest : ModifyRequest
{
  ...
  void ParseFrom(string_view input) override
  {
    ...
    value = static_cast<size_t>(ConvertToInt(input));
  }
...
struct PayTaxRequest : ModifyRequest
{
  ...
  void Process(BudgetManager& manager) const override
  {
    manager.GetTree()->AddBulkOperation(
      ...
      BulkLinearUpdater(manager.GetTree(),
...
RequestHolder
Request::Create(Request::Type type)
{
  switch (type) {
...
template<typename Number>
Number
ReadNumberOnLine(istream& stream)
{
  Number number;
  stream >> number;
  string dummy;
...
ConvertRequestTypeFromString(string_view type_str)
{
...
RequestHolder
ParseRequest(string_view request_str)
{
...
vector<RequestHolder>
ReadRequests(istream& in_stream = cin)
{
...
vector<double>
ProcessRequests(const vector<RequestHolder>& requests)
{
...
void
PrintResponses(const vector<double>& responses, ostream& stream = cout)
{
...
test_simple()
{
  TestDataHandler test_data(STRINGIFY2(TESTING_DIR_personal_budget_sanitize),
                            "in_simple.txt",
                            "out_simple.txt");

  ostringstream out;

  const auto requests = ReadRequests(test_data.InputData());
  const auto responses = ProcessRequests(requests);
  PrintResponses(responses, out);

  ASSERT_EQUAL(out.str(), ReadWholeInput(test_data.ReferenceData()));
}
