...
namespace {
bool
operator==(const Parse::Token& token, char c)
{
  ...
  unique_ptr<Ast::Statement> ParseMult()
  {
    if (lexer.CurrentToken() == '(') {
      ...
      if (lexer.CurrentToken() == '(') {
        ...
        if (!names.empty()) {
          return make_unique<Ast::MethodCall>(
            make_unique<Ast::VariableValue>(std::move(names)), std::move(method_name), std::move(args));
...
unique_ptr<Ast::Statement>
ParseProgram(Parse::Lexer& lexer)
{
