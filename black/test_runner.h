...
template<class Map>
std::ostream&
PrintMap(std::ostream& os, const Map& m)
{
  os << "{";
  bool first = true;
  for (const auto& kv : m) {
...
template<class K, class V>
std::ostream&
operator<<(std::ostream& os, const std::unordered_map<K, V>& m)
{
  return TestRunnerPrivate::PrintMap(os, m);
}
...
#define ASSERT_THROWS(expr, exception)                                                                                 \
  do {                                                                                                                 \
    try {                                                                                                              \
      expr;                                                                                                            \
      std::ostringstream __assert_private_os;                                                                          \
      __assert_private_os << "Expression " #expr " is expected to throw " #exception " " FILE_NAME ":" << __LINE__;    \
