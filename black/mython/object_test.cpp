...
TestFields()
{
  ...
  methods.push_back({ "add",
                      { "x" },
                      { make_unique<Ast::FieldAssignment>(
                        Ast::VariableValue{ "self" },
                        "value",
                        make_unique<Ast::Add>(make_unique<Ast::VariableValue>(vector<string>{ "self", "value" }),
...
{
  vector<Method> methods;
  methods.push_back({ "GetValue", {}, make_unique<Ast::VariableValue>(vector{ "self"s, "value"s }) });
  methods.push_back({ "SetValue",
                      { "x" },
                      make_unique<Ast::FieldAssignment>(
...
TestInheritance()
{
  ...
  methods.clear();
  methods.push_back({ "GetValue", { "z" }, make_unique<Ast::VariableValue>("z") });
  methods.push_back({ "AsString", {}, make_unique<Ast::StringConst>("value"s) });
